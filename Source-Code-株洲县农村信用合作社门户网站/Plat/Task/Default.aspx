<%@ page language="C#" autoeventwireup="true" inherits="Plat_Task_Default, App_Web_crwofczn" masterpagefile="~/Plat/Main.master" enableEventValidation="false" viewStateEncryptionMode="Never" %>
...
<asp:Content runat="server" ContentPlaceHolderID="Content">
    ...
    <div  style="margin-top:10px;margin-bottom:10px;">
        <div class="input-group nav_searchDiv" style="float:right;">
            ...
            <div class="iconDiv" title="创建新任务" onclick="$('#modal1').click();">
             ...
             <ZL:ExGridView runat="server" ID="EGV" AutoGenerateColumns="false" AllowPaging="true" PageSize="10"  EnableTheming="False" IsHoldState="false"
                ...
                <Columns>
                    ...
                    <asp:TemplateField HeaderText="任务名">
                        <ItemTemplate>
                             <a href="javascript:;" title="查看详情" onclick="ViewTaskDetail(<%#Eval("ID") %>);"><%#Eval("TaskName") %></a>
                        </ItemTemplate>
                    </asp:TemplateField>
                    ...
                    <asp:TemplateField  HeaderText="任务色彩">
                        <ItemTemplate>
                                <span id="task_Color" class="colorSpan_F">
                                    <span  class="colorSpan" id="colorSpan_<%#Eval("ID") %>"  onclick="ShowColor(this)" style="background-color:<%#Eval("Color") %>; border: 1px solid <%#Eval("Color") %>;"></span>
                                </span>
                            <div class="ex_Color" onclick="HideColor()">
                                <span class="colorSpan_F">
                                    <span class="colorSpan"  onclick="white(<%#Eval("ID") %>,'CCCCCC')" style="background-color: #CCCCCC; border: 1px solid #ddd;"></span>
                                </span>
                                <span class="colorSpan_F">
                                    <span class="colorSpan" onclick="white(<%#Eval("ID") %>,'3BB3FF')" style="background-color: #3bb3ff; border: 1px solid #0084ff;"></span>
                                </span>
                                <span class="colorSpan_F">
                                    <span class="colorSpan" onclick="white(<%#Eval("ID") %>,'9F74FF')" style="background-color: #9f74ff; border: 1px solid #5d16ff"></span>
                                </span>
                                <span class="colorSpan_F">
                                    <span class="colorSpan" onclick="white(<%#Eval("ID") %>,'FF7C7C')"style="background-color: #ff7c7c; border: 1px solid red;"></span>
                                </span>
                                <span class="colorSpan_F">
                                    <span class="colorSpan"onclick="white(<%#Eval("ID") %>,'FFC85D')" style="background-color: #ffc85d; border: 1px solid #ff9600;"></span>
                                </span>
                                <span class="colorSpan_F">
                                    <span class="colorSpan" onclick="white(<%#Eval("ID") %>,'FFF65F')" style="background-color: #fff65f; border: 1px solid #eee100;"></span>
                                </span>
                            </div>
                        </ItemTemplate>
                    </asp:TemplateField>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                  <table class="table table-bordered">
                       <tr><td>任务名称：</td><td><asp:TextBox  runat="server" ID="TaskName_T" class="form-control required" /></td></tr>
                       <tr><td>主负责人：</td><td>
                           <input type="text" id="LeaderIDS_T" class="form-control required"/><input type="button" value="选择" onclick="selRuser();" class="btn btn-primary " style="margin-left:5px;" />
                           <asp:HiddenField runat="server" ID="LeaderIDS_Hid" />
                                         </td></tr>
                       <tr><td>参与成员：</td><td>
                           <input type="text" id="PartTakeIDS_T"class="form-control required"/><input type="button" value="选择" onclick="selCuser();" class="btn btn-primary " style="margin-left:5px;"/>
                       ...
                       <tr><td>截止日期：</td><td><asp:TextBox runat="server" ID="EndTime_T" onfocus="WdatePicker({dateFmt:'yyyy/MM/dd HH:mm'});" CssClass="form-control required date" /></td></tr>
                       <tr><td>任务颜色：</td><td>
                           <span class="colorSpan_F">
...
<asp:Content runat="server" ContentPlaceHolderID="ScriptContent">
    ...
    <script type="text/javascript">
        ...
        function white(id, color) {
            document.getElementById('colorSpan_' + id).style.backgroundColor = '#' + color;
            document.getElementById('colorSpan_' + id).style.borderColor = '#' + color;
            document.getElementById('colorSpan_' + id).style.hoverColor = '#' + color;
            PostReName(id)
        }
        ...
        function PostReName(id) {
            var color = document.getElementById('colorSpan_' + id).style.backgroundColor.toString();
            var ids = id;
            PostToCS(color, ids, function (data) {
            });
        }
    </script>
</asp:Content>
